...
type (
	ConfigurationUsecase interface {
...
func NewUsecase(repo ConfigurationRepository) ConfigurationUsecase {
	return &usecase{
		repo: repo,
	}
}
...
func (uc *usecase) GetHD(c *gin.Context) {
	data, err := uc.repo.GetDataBy(c, "name", "help_desk")
	if err != nil {
	...
	util.JOK(c, http.StatusOK, data.Value)
}
...
func (uc *usecase) Create(c *gin.Context) {
	var req ConfigurationCreateRequest

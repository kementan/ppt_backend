...
type (
	MenuRepository interface {
		GetDataBy(c context.Context, field string, value string) (MenuResponse, error)
		GetTable(c context.Context, arg util.DataFilter) ([]MenuResponse, error)
		CountRecords(c context.Context, arg util.DataFilter) (int, error)
		Read(c context.Context) ([]MenuResponse, error)
		Create(c context.Context, arg MenuCreate) (MenuResponse, error)
		Update(c context.Context, id string, arg MenuUpdate) (MenuResponse, error)
...
func NewRepository(db *sql.DB) MenuRepository {
	return &repository{
		db: db,
	}
}

func (q *repository) GetDataBy(c context.Context, field string, value string) (MenuResponse, error) {
	var r MenuResponse
	var enc_id string
...
func (q *repository) GetTable(c context.Context, arg util.DataFilter) ([]MenuResponse, error) {
	...
	query := `
		...
		` + table + `.sort, 
		` + table + `.is_active, 
		COUNT(` + subtable + `.id) AS sub_menu, 
		` + table + `.created_at, 
		` + table + `.updated_at
		FROM ` + table + `
		LEFT JOIN ` + subtable + ` ON ` + subtable + `.menu_id = ` + table + `.id
		GROUP BY ` + table + `.id
		`
...
func (q *repository) Read(c context.Context) ([]MenuResponse, error) {
	items := []MenuResponse{}
...
func (q *repository) Create(c context.Context, arg MenuCreate) (MenuResponse, error) {
	var r MenuResponse
	var enc_id string
...
func (q *repository) Update(c context.Context, id string, arg MenuUpdate) (MenuResponse, error) {
	var r MenuResponse
